...
class SendPush(FirebaseBase):
    async def update_user_info_rates(self, source_cur, targets, rate_data, user_info):
        for current in self.api_local[rate_data]:
            if current == source_cur:
                for val in self.api_local[rate_data][current]:
    ...
    async def main(self):
        ...
        tasks = [self.process_token(token, user_data) for token, user_data in self.users_local.items() if
                 user_data is not None]
        await asyncio.gather(*tasks)

    def cleanup(self):
        for token, user_data in self.users_local.items():
